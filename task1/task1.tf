resource "google_storage_bucket" "armageddon-bucket1" {
  name          = "yakgeddon1"
  location      = "US"
  force_destroy = true
...
resource "google_storage_bucket_acl" "bucket_acl" {
    bucket         = google_storage_bucket.armageddon-bucket1.name
...
output "website_url" {
    value = "https://storage.googleapis.com/${google_storage_bucket.armageddon-bucket1.name}/index.html"
}
